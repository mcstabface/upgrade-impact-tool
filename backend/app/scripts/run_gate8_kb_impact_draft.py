...
PIPELINE_STEPS = [
    ...
    PipelineStep(
        label="Generate constrained citation-bound KB impact draft",
        module="app.scripts.generate_kb_impact_draft",
        args=[],
    ),
    PipelineStep(
        label="Validate constrained KB impact draft",
        module="app.scripts.validate_kb_impact_draft",
        args=[],
    ),
    PipelineStep(
        label="Write KB impact draft summary report",
        module="app.scripts.write_kb_impact_draft_summary",
        args=[],
    ),
]
...
def run_step(step: PipelineStep, *, dry_run: bool) -> None:
    ...
    print(f"[gate8] {step.label}")
    print(f"[gate8]   {' '.join(command)}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Gate 8 constrained KB impact draft generation pipeline.")
...
def main() -> None:
    ...
    print("[gate8] Starting constrained KB impact draft pipeline")
    print(f"[gate8] Repository root: {repository_root}")

    for step in PIPELINE_STEPS:
        run_step(step, dry_run=args.dry_run)

    if args.dry_run:
        print("[gate8] Dry run complete")
        return

    missing_outputs = verify_outputs(repository_root)
    if missing_outputs:
        print("[gate8] Pipeline completed, but expected outputs are missing:")
        for output in missing_outputs:
            print(f"[gate8]   missing: {output}")
        raise SystemExit(1)

    print("[gate8] Pipeline complete")
    print("[gate8] Outputs:")
    for output in EXPECTED_OUTPUTS:
        print(f"[gate8]   {output}")
